/**
 * `/console/types-de-fiches` — LE CHARGEUR de V-29. LA GARDE EST CELLE DES ONZE ADRESSES
 * DE CONSOLE : `resoudreLaConsole()` la prend, une fois pour toutes, et un
 * non-administrateur reçoit 404 V-26 — pas un refus (`P-09`, `RG-ACC-04`). Le seul
 * `error(404, …)` est SANS MESSAGE (`ADR-007`).
 *
 * LE SEGMENT D'ADRESSE EST CELUI DE `docs/routes.md` §3.6, non le vocabulaire de la vue :
 * le concept « fiche » est renommable globalement par la configuration (`P-07`), et
 * l'ADRESSE ne se renomme pas — la déduire du réglage rendrait un chemin instable.
 *
 * `typesFiche` est une propriété REQUISE de V-29, servie depuis `types_de_fiche` et
 * `champs_de_type_de_fiche` : sans cela, une page qui oubliait de la passer servait trois
 * types qui n'existaient nulle part.
 */
...
export const actions: Actions = {
	 ...
	 *
	 * `type-de-fiche` porte l'IDENTIFIANT LISIBLE du type, que la colonne
	 * `types_de_fiche.identifiant` rend unique. Le gel désigne le type par son nom
	 * dans une fermeture et n'expose aucun nom de champ ; la clé retenue est celle
	 * par laquelle le schéma le désigne, jamais son libellé — un libellé se renomme,
	 * un identifiant lisible est stable.
	 *
	 * LE REFUS PORTE LE NOMBRE ET LA SORTIE, les deux que la règle exige.
	 */
	...
	/**
	 * DÉLESTER LES NOTES D'UN TYPE — la sortie que le refus propose. Le gel porte le
	 * bouton, et `P-03` interdit qu'il soit inerte.
	 *
	 * Le type est désigné par le MÊME champ que la suppression — `type-de-fiche` —
	 * parce que c'est le même objet. Deux noms de champ pour une même clé finiraient
	 * par diverger.
	 */
	 ...
	 *
	 * DEUX TABLES, UNE TRANSACTION : `types_de_fiche` et `champs_de_type_de_fiche`.
	 * Un type créé sans ses propriétés serait un schéma vide que l'administrateur
	 * croirait avoir défini.
	 *
	 * LES CINQ ATTRIBUTS QUE CE PANNEAU JETAIT SONT ÉCRITS : description, icône, et
	 * par propriété l'aide à la saisie, la valeur par défaut et le caractère
	 * obligatoire n'avaient AUCUNE colonne — l'administrateur les saisissait,
	 * l'écran les affichait comme acquises, et l'enregistrement les perdait sans un
	 * mot.
	 */
