/**
 * Recherche et mise en évidence — port fidèle des fabriques des maquettes gelées de
 * l'espace public et de la page non trouvée connectée : `window.chercher()`,
 * `window.chercherPublic()` et `window.surligner()` (`V-01:1215` et suivantes, mêmes
 * corps dans V-02, V-03, V-04, V-26). Ce qui n'est pas dans le gel n'est pas ici.
 *
 * PÉRIMÈTRE PUBLIC — `RG-M17-01`, ET IL EST APPLIQUÉ AILLEURS QU'ICI. Les vues
 * publiques réduisent leur corpus AU POINT D'ENTRÉE de la vue, jamais au moment de
 * l'affichage. Ce module ne filtre donc RIEN de lui-même : il cherche dans le jeu
 * qu'on lui donne. Une fonction qui filtrerait ici déplacerait la contrainte hors de
 * son point d'application et permettrait qu'un appel l'oublie.
 *
 * Ce module rend un état de maquette ; il ne résout aucun droit.
 */
import type { Note } from '../../../seeds/corpus';

/**
 * LE PÉRIMÈTRE PUBLIC, ET IL VIT DÉSORMAIS ICI. `notesPubliques()` était importée de
 * `seeds/corpus.ts` par V-01, V-02 et V-04 : un appel de fonction du jeu de
 * démonstration au point d'entrée de trois écrans servis sans session, dont la forme
 * admettait `CORPUS` en argument par défaut — un appel sans argument rendait le corpus
 * de démonstration.
 *
 * Ici l'argument est EXIGÉ, et la valeur comparée est typée contre
 * `Note['visibilite']` : renommer la valeur dans le corpus ne peut pas laisser ce
 * filtre en silence sur une constante devenue fausse.
 */
...
/**
 * Découpe un texte selon les termes de la requête, sans jamais injecter de balisage
 * issu de la saisie — la propriété que `window.surligner()` obtient en construisant
 * des nœuds de texte, et que ce port obtient en rendant des segments que Svelte
 * échappe. Seuls les termes de plus de deux caractères marquent, comme au gel.
 */
