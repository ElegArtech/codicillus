/**
 * LES QUINZE CONSTRUCTIONS, VUES DEPUIS L'ÉDITEUR — ce qu'il sait produire, ce qu'il
 * insère, et ce qui lui manque. `contenu/document.ts` porte le catalogue
 * `CONSTRUCTIONS` (porteurs et signatures) ; ce module-ci répond à l'autre question,
 * *l'éditeur sait-il les produire ?* Il les LIT, il n'en redéfinit aucune.
 *
 * LES GABARITS D'INSERTION SONT DU GEL : `V-17:3068-3084` porte, pour chaque clé de
 * bouton, le fragment que l'éditeur insère. Ils sont TRANSCRITS ici au format
 * canonique, jamais réinventés — un gabarit choisi par l'implémenteur serait la
 ...
 *     porte pas de saut de ligne dur — règle 4 —, et le paragraphe vide s'écrit par
 *     un paragraphe sans contenu, ce que la règle 1 prévoit.
 *   · les quatre marques de caractère n'ont AUCUN gabarit : le gel les applique à la
 *     sélection courante (`V-17:3099-3105`). Leur fragment marqué reprend donc le nom
 *     accessible du bouton (`V-17:1522-1531`), seule lecture disponible.
 *
 * TROIS CONSTRUCTIONS PORTENT UNE LACUNE, ET ELLES SONT COMPTÉES : `LACUNES` les
 * nomme, `constructions.test.ts` compte. Une lacune qui disparaîtrait sans que la
...
/**
 * LES QUATORZE GABARITS D'INSERTION FIXES, dans l'ordre de la barre gelée. La table
 * du gel porte QUINZE clés ; douze sont ici telles quelles, les trois autres — image,
 * diagramme, lien interne — ont besoin d'une valeur que le gel ne donne pas et sont
 * des FABRIQUES plus bas. S'ajoutent les deux listes, que le gel produit par une
 * commande d'édition sans gabarit : 12 + 2 = 14.
 */
...
/**
 * LE LIEN INTERNE — le gel écrit le TITRE de la note dans le texte et ne porte aucune
 * cible (`V-17:3313-3314`) ; `ADR-003` impose l'IDENTIFIANT dans la marque. Les deux
 * ensemble donnent cette fabrique, et c'est elle qui rend le lien insensible au
 * renommage.
 */
...
/**
 * LES TROIS LACUNES, NOMMÉES ET COMPTÉES. Ce ne sont pas des refus d'implémenter :
 * ce sont des vides que rien dans les sources ne comble, et qu'un implémenteur ne
 * peut combler qu'en décidant à la place du commanditaire.
 */
...
/**
 * LE COMPTE — combien de constructions l'éditeur produit sans réserve, et combien
 * portent une lacune nommée. Calculé, jamais recopié : un en-tête recopié annonçait
